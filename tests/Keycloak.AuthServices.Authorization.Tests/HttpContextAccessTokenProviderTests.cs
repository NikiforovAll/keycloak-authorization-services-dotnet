...
public class HttpContextAccessTokenProviderTests
{
    ...
    [Fact]
    public async Task GetAccessTokenAsync_DefaultScheme_UsesSourceAuthenticationScheme()
    {
        var options = new KeycloakAuthorizationServerOptions
        {
            SourceAuthenticationScheme = "Bearer",
            SourceTokenRetrievalScheme = null,
        };
        var (provider, contextAccessor) = CreateProvider(options, BuildHttpContext("Bearer", "access_token", TestToken));
    ...
    [Fact]
    public async Task GetAccessTokenAsync_SourceTokenRetrievalScheme_UsesCookieScheme()
    {
        var options = new KeycloakAuthorizationServerOptions
        {
            SourceAuthenticationScheme = "Bearer",
            SourceTokenRetrievalScheme = "Cookies",
        };
    ...
    [Fact]
    public async Task GetAccessTokenAsync_SourceTokenRetrievalSchemeSet_DoesNotUseFallbackScheme()
    {
        var options = new KeycloakAuthorizationServerOptions
        {
            SourceAuthenticationScheme = "Bearer",
        ...
        // Token is stored under "Bearer" but retrieval scheme is "Cookies" — should return null
    ...
    [Fact]
    public async Task GetAccessTokenAsync_EmptySourceTokenRetrievalScheme_UsesSchemelessOverload()
    {
        var options = new KeycloakAuthorizationServerOptions
        {
            SourceAuthenticationScheme = "Bearer",
            SourceTokenRetrievalScheme = string.Empty,
        };
        var httpContext = BuildHttpContext(null, "access_token", TestToken);
        var (provider, _) = CreateProvider(options, httpContext);
    ...
    /// <summary>
    /// Builds a fake <see cref="HttpContext"/> that returns <paramref name="token"/> from
    /// <see cref="IAuthenticationService.AuthenticateAsync"/> for the specified
    /// <paramref name="scheme"/> and <paramref name="tokenName"/>.
    /// When <paramref name="scheme"/> is <see langword="null"/>, the token is stored in the
    /// default (scheme-less) authentication result.
    /// </summary>
    private static HttpContext BuildHttpContext(string? scheme, string tokenName, string token)
    {
        ...
        var authResult = AuthenticateResult.Success(
            new AuthenticationTicket(new System.Security.Claims.ClaimsPrincipal(), props, scheme ?? "__default__")
        );
        ...
        services.AddSingleton<IAuthenticationSchemeProvider>(
            new FakeSchemeProvider(scheme ?? "__default__")
        );
    ...
    ) : IAuthenticationService
    {
        ...
        {
            // Return success only when the scheme matches (or for scheme-less lookup)
            if (scheme == expectedScheme || (expectedScheme == null && scheme == null))
            {
